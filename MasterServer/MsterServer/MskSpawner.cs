...
namespace Msk.Master
{
    public class MskSpawner
    {
        ...
        public class SpawnRequest
        {
            ...
            /// </summary>
            public KeyValuePair<int, string> RequestedBy { get; private set; }
            ...
            public SpawnRequest(Process process, ushort port, MskSocket client, string version, string roomName)
            {
                ...
                this.RequestedBy = new KeyValuePair<int, string>(client.ClientId, client.UUID);
        ...
        {
            for (int i = 0; i < MasterServerKit.Master.Config.MaxInstanceCount; i++)
            {
                ushort port = (ushort)(i + MasterServerKit.Master.Config.PortStart);
                Ports.Enqueue(port);
            }
        }
        ...
        public static int GetPlayerClientRequestedCreatingRoom(string version, string roomName)
        {
            SpawnRequest spawnRequest = FindSpawnRequest(version, roomName);
            if (spawnRequest != null)
            {
                return spawnRequest.RequestedBy.Key;
            }
            ...
            RequestHandlerBase requestHandlerBase = null)
        {
            //try
            {
                ...
                if (spawnRequest != null)
                {
                    ...
                    if (spawnRequest.RequestedBy.Value == client.UUID)
                    {
                ...
                Config config = MasterServerKit.Master.Config;

                string ip = config.MasterHost;
                ushort port = Ports.Dequeue();

                string masterIp = config.MasterHost;
                ushort masterPort = config.MasterPort;
                string path = $"{config.ServerInstancePath}/" + (config.UseVersionInInstancePath ? $"{version}/" : "")
                    + config.InstanceFileName;
        ...
        public static void RemoveRoomProcess(int RoomClientId, string version)
        {
            ...
            for (int i = 0; i < processes.Count; i++)
            {
                ...
                if (roomProcess.RoomClientId == RoomClientId)
                {
                    ...
                    if (!Ports.Contains(roomProcess.Port))
                    {
